...
public class Util {
    ...
    public static double applyFrictions(double force, double velocity, double STATIC_FRIC, double KINE_FRIC,
        ...
        if (Math.abs(velocity) < FRIC_THRESHOLD && Math.abs(force) < STATIC_FRIC) { // if not moving, apply static
                                                                                    // friction
            return 0;
        }
    ...
    public static double posModulo(double input, double modulo) { // modulo but it always returns a positive number,
                                                                  // ideal for screen loopback
        while (input >= modulo)
            input -= modulo;
        while (input < 0)
            input += modulo;
    ...
    public static class Pose2D extends Vector2D {
        ...
        public Pose2D(Vector2D vector2D, double heading_input){
            x = vector2D.x;
            y = vector2D.y;
        ...
        public Vector2D getVector2D(){
            return new Vector2D(x, y, Type.CARTESIAN);
        }

        public Pose2D add(Pose2D step) {
            return new Pose2D(this.add(step.getVector2D().rotate(heading)), heading + step.heading);
        }

        public String toString(){
            return "(" + Util.roundHundreths(x) + ", " + Util.roundHundreths(y) + ", " + Util.roundHundreths(heading) + ")";
        }
    }
